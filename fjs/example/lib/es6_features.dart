...
class ES6FeaturesCheck extends StatefulWidget {
  const ES6FeaturesCheck({Key? key}) : super(key: key);

  @override
  _ES6FeaturesCheckState createState() => _ES6FeaturesCheckState();
}

class _ES6FeaturesCheckState extends State<ES6FeaturesCheck> {
  Map<String, bool> results = {};
  List<List<String>> data = [
    ['Constants', Constants],
    ['BlockScopedVariables', BlockScopedVariables],
    ['BlockScopedFunctions', BlockScopedFunctions],
    ['ArrowExpressBodies', ArrowExpressBodies],
    ['ArrowStatementBodies', ArrowStatementBodies],
    ['ArrowLexicalThis', ArrowLexicalThis],
    ['DefaultParameterValues', DefaultParameterValues],
    ['RestParameter', RestParameter],
    ['SpreadOperator', SpreadOperator],
    ['StringInterpolation', StringInterpolation],
    ['CustomInterpolation', CustomInterpolation],
    ['RawStringAccess', RawStringAccess],
    ['BinaryAndOctalLiteral', BinaryAndOctalLiteral],
    ['UnicodeStringAndRegExpLiteral', UnicodeStringAndRegExpLiteral],
    ['RegularExpressionStickyMatching', RegularExpressionStickyMatching],
    ['PropertyShorthand', PropertyShorthand],
    ['ComputedPropertyNames', ComputedPropertyNames],
    ['MethodProperties', MethodProperties],
    ['ArrayMatching', ArrayMatching],
    ['ObjectMatchingShorthandNotation', ObjectMatchingShorthandNotation],
    ['ObjectDeepMatching', ObjectDeepMatching],
    ['ObjectAndArrayMatchingDefaultValues', ObjectAndArrayMatchingDefaultValues],
    ['ParameterContextMatching', ParameterContextMatching],
    ['FailSoftDestructuringOptionallyWithDefaults', FailSoftDestructuringOptionallyWithDefaults],
    ['ValueExport', ValueExport],
    ['ValueImport', ValueImport],
    ['DefaultAndWildcardExport', DefaultAndWildcardExport],
    ['DefaultAndWildcardImport', DefaultAndWildcardImport],
    ['ClassDefinitionAndInheritance', ClassDefinitionAndInheritance],
    ['ClassInheritanceFromExpressions', ClassInheritanceFromExpressions],
    ['BaseClassAccess', BaseClassAccess],
    ['StaticMembers', StaticMembers],
    ['GetterSetter', GetterSetter],
    ['SymbolType', SymbolType],
    ['GlobalSymbols', GlobalSymbols],
    ['IteratorAndForOfOperator', IteratorAndForOfOperator],
    ['GeneratorFunctionAndIteratorProtocol', GeneratorFunctionAndIteratorProtocol],
    ['GeneratorFunctionDirectUse', GeneratorFunctionDirectUse],
    ['GeneratorMatching', GeneratorMatching],
    ['GeneratorControlFlow', GeneratorControlFlow],
    ['GeneratorMethods', GeneratorMethods],
    ['SetDataStructure', SetDataStructure],
    ['MapDataStructure', MapDataStructure],
    ['WeakLinkDataStructures', WeakLinkDataStructures],
    ['TypedArrays', TypedArrays],
    ['ObjectPropertyAssignment', ObjectPropertyAssignment],
    ['ArrayElementFinding', ArrayElementFinding],
    ['StringRepeating', StringRepeating],
    ['StringSearching', StringSearching],
    ['NumberTypeChecking', NumberTypeChecking],
    ['NumberSafetyChecking', NumberSafetyChecking],
    ['NumberComparison', NumberComparison],
    ['NumberTruncation', NumberTruncation],
    ['NumberSignDetermination', NumberSignDetermination],
    ['PromiseUsage', PromiseUsage],
    ['PromiseCombination', PromiseCombination],
    ['PromiseCombinationSimple', PromiseCombinationSimple],
    ['Proxying', Proxying],
    ['Reflection', Reflection],
    ['Collation', Collation],
    ['NumberFormatting', NumberFormatting],
    ['ConcurrencyFormatting', ConcurrencyFormatting],
    ['DateAndTimeFormatting', DateAndTimeFormatting],
  ];
    ...
    return Scaffold(
      ...
      body: ListView(
        children: data.map((_) => ListTile(
          title: Text(_[0]),
          onTap: ()  => eval(_[0], _[1]),
          tileColor: resultToColor(results[_[0]]),
        )).toList(),
      ),
    );
  }

  Color? resultToColor(bool? result) {
    if(result == null) {
      return null;
    }
  ...
  void eval(String name, String code) async {
    ...
    try {
      ...
      vm.startEventLoop();
      await Future.value(vm.jsToDart(vm.evalCode(code, filename: name)));
      results[name] = true;
    } catch(e, s) {
      results[name] = false;
      print('$name\n$e\n$s');
...
const DefaultParameterValues = r'''
...
const BinaryAndOctalLiteral = r'''
expect(0b111110111, 503)
expect(0o767, 503)
''';

const UnicodeStringAndRegExpLiteral = r'''
expect("𠮷".length, 2)
...
const RegularExpressionStickyMatching = r'''
...
const PropertyShorthand = r'''
var x = 0, y = 1
var obj = { x, y }
expect(obj, {x: 0, y: 1})
''';

const ComputedPropertyNames = r'''
let obj = {
    foo: "bar",
    [ "baz" + "z" ]: 42
}
...
const ObjectMatchingShorthandNotation = r'''
function getASTNode() {
  return {
    op: {op: 1},
    lhs: {op:2},
...
const ObjectAndArrayMatchingDefaultValues = r'''
var obj = { a: 1 }
var list = [ 1 ]
var { a, b = 2 } = obj
expect(a, 1);
...
const ParameterContextMatching = r'''
function f ([ name, val ]) {
...
const FailSoftDestructuringOptionallyWithDefaults = r'''
var list = [ 7, 42 ]
...
const DefaultAndWildcardExport = r'''
//  lib/mathplusplus.js
...
const DefaultAndWildcardImport = r'''
import exp, { pi, e } from "lib/mathplusplus"
expect("e^{π} = " + exp(pi), 'e^{π} = 23.140700648952773')
  ''';

const ClassDefinitionAndInheritance = r'''
class Shape {
    constructor (id, x, y) {
        this.id = id
        this.move(x, y)
...
const ClassInheritanceFromExpressions = r'''
...
const IteratorAndForOfOperator = r'''
let fibonacci = {
    [Symbol.iterator]() {
        let pre = 0, cur = 1
        return {
...
const GeneratorFunctionAndIteratorProtocol = r'''
let fibonacci = {
    *[Symbol.iterator]() {
        let pre = 0, cur = 1
        for (;;) {
...
const GeneratorFunctionDirectUse = r'''
...
const WeakLinkDataStructures = r'''
...
const ObjectPropertyAssignment = r'''
var dest = { quux: 0 }
...
const PromiseCombinationSimple = r'''
(async function() {
var actual = await Promise.all([
